...
export default function InDeliveryOrderFormDialog({
  open,
  setOpen,
}: BoxDetailDialogProps) {
  const { isEditing, order } = useAppSelector((state) => state.order);
  ...
  return (
    ...
    >
      <Formik
        ...
        //     ? order
        //     : {
        //         followedLink: isEditing ? order?.followedLink : '',
        //       }
        // }
        initialValues={{
          followedLink: isEditing ? order.followedLink : '',
        ...
        validationSchema={Yup.object({
          followedLink: Yup.string().required('Champ obligatoire'),
        })}
            ...
            <DialogTitle id="alert-dialog-title">
              <div className="w-full flex items-center justify-between ">
                <Typography variant="h5" color="primary">
                  Mettre la commande en cours de livraison
                </Typography>
                <MuiContainedButton
                  startIcon={<Close />}
                  variant="contained"
            ...
            <DialogContent>
              <div className="flex lg:flex-row md:flex-row sm:flex-col w-full py-2">
                ...
                <div className="h-full p-2.5 w-full flex flex-col gap-2.5 justify-between lg:border-r lg:border-r-gray-100">
                  {/* <KeyValue
                    title="Adresse"
                    value={order.shoppingCart?.user?.address}
                  /> */}
                  {/* <KeyValue title="Numero commande" value="Cm002" /> */}
                  {/* <KeyValue title="Autre" value="Lorem upsum " /> */}
                  <KeyValue
                    title="Adresse"
                ...
                <div className="h-full p-2.5 w-full flex flex-col gap-2.5 justify-between">
                  <OSTextField
                    name="followedLink"
                    size="small"
                    label="Lien suivie de livraison"
                  />
                </div>
              </div>
            </DialogContent>
            ...
            <DialogActions>
              <div className="w-full flex gap-2.5 justify-end px-4">
                ...
                <MuiContainedButton
                  ...
                  onClick={() => handleClickOnDelivery(order.id)}
                  type="submit"
                >
                  Enregistrer
                </MuiContainedButton>
